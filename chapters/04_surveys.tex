...
Nous présentons dans ce chapitre les sondages dont nous utilisons les données ou
qui apparaissent dans cette thèse. Les cinq premiers constituent notre
échantillon de base, alors que les deux derniers ont une présence limitée dans
notre étude. Un comparatif des caractéristiques des sondages est présenté
Tableauau~\ref{tab:sondcomp}.
...
fortement de celui du sondage SNLS, cf Section~\ref{sec:snls}.
...
celui de SDSS (cf. Section~\ref{sec:sdss}), mais la spécificité de SNLS est de
...
opérations en 2018. Comme d'autres sondages, plusieurs programmes composent ce
relevé~; pour notre étude, c'est le \textit{Bright Transient Survey} (BTS) qui
nous intéresse. Il se place comme le nouveau sondage de référence à bas
redshift, permettant de remplacer les données ciblées des sondages CfA et CSP et
la multitude d'instruments de SNf par un seul échantillon homogène à recherche
glissante.
...
l'échantillonnage pré-explosion. Les transmissions des filtres et un exemple de
courbe de lumière sont présentés Figures~\ref{fig:ztfbands} et \ref{fig:ztflc}. 
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[]{.49\linewidth}
        ...
        \captionsetup{justification=centering}
        \caption{Courbe de lumière en bandes $gri$ de la SN~Ia confirmée
        ZTF19aantarc, à $z = 0,099$.}
        \label{fig:ztflc}
    \end{subfigure}
